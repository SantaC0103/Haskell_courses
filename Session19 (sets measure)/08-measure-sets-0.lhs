...
\begin{code}
import Data.Set (Set, difference, empty, intersection, union) //后面改
...
--Exercise 1: give a refined type for revHelper//已解决
...
\end{code}
 ...
 --Exercise 2: give a refined type for halve//已解决
...
\begin{code}
import Data.Set (Set, empty, singleton, union, member) //后面改
...
\end{code}
 ...
 --Exercise 3: give a refined type for elem//已解决
...
\begin{code}
import Data.Set (Set, empty, singleton, union) //后面改
{-@ lazy mergeSort @-}
...
\end{code}
 ...
 -- to fix a type for halve (Exercise 2)//已解决
...
-- Exercise 6: give filter' a refined type//已解决
...
-- Exercise 8: refine the type of append to ensure a unique list as output //已解决，但isin没找到
...
focusLeft                 :: Eq a => Zipper a -> Zipper a
focusLeft (Zipper t (l:ls) rs) = Zipper l ls (t:rs)
focusLeft (Zipper t [] rs)     = --assert (not (isin x xs)) $
                                 Zipper x xs []
  where  
    (x:xs)   = reverse (t:rs)
\end{code}
...
\begin{code}
focusRight    :: Eq a => Zipper a -> Zipper a
